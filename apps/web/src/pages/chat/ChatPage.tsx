...
export function ChatPage() {
  ...
  const senderCacheKey = useMemo(() => getSenderCacheKey(senderAddresses), [senderAddresses]);
  ...
  const fetchSentMessages = useCallback(
    async ({
      ...
      try {
        ...
        if (cacheKey) {
          chatPageSentCheckpointCache.set(cacheKey, lastSentSyncedBlockRef.current);
        }
  ...
  useEffect(() => {
    ...
    lastSentSyncedBlockRef.current = chatPageSentCheckpointCache.get(senderCacheKey) ?? null;
    setSentLogError(null);
    setIsSentLogLoading(false);
